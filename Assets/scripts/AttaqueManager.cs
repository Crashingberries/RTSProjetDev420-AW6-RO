...
public class AttaqueManager : MonoBehaviour {
    ...
    void Start () {
        ...
        PvBar = gameObject.transform.Find("Canvas").transform.Find("Slider").GetComponent<Slider>();
    }
	
	// Update is called once per frame
	void Update () {
        Debug.Log(PvBar.value);
    ...
    public void UpdateCilblesPossibles()
    {
        ...
        for (int i = 0; i < ennemis.Length; i++)
        {
            if (ennemis[i].gameObject.GetComponent<AttaqueManager>() == null || ennemis[i].gameObject == attaquant ||ennemis[i].gameObject.tag ==attaquant.tag)
            {
                ennemis[i] = null;
            }
        }
    ...
    {
        Debug.Log("Liste des Ennemis::" + ennemis.Length);
        //Debug.Log("AttaqueManager::GetCible ==> Nom de l'ennemi AVANT LA BOUCLE " + ennemis[0].name);
        for (int i = 0; i < ennemis.Length; i++)
        {
            //Debug.Log("AttaqueManager::GetCible ==> Nom de l'ennemi "+ennemis[i].name);
    ...
    public void Attaquer(AttaqueManager _cible)
    {
        if (_cible != null)
        {
            ...
            {
                Debug.Log("Attaque : " + cible.name + ", PV : " + _cible.PvBar.name);
                ...
                if (_cible.PvBar.value >= 0)
                {
                    ...
                    if (true)
                    {
                        ...
                        Debug.Log("AttaqueManager::Attaquer ==>"+_cible.PvBar.value + " Attaque: "+ ATTAQUE);
                    }
